    ...
    def _get_accounts_with_move_lines(self, options):
        ...
        query = SQL(
            """
                ...
                WHERE account.company_id IN %(company_ids)s
                AND (
                    account.account_type = 'equity_unaffected'
                    OR EXISTS (
                        SELECT 1
                        FROM account_move_line aml
                        WHERE aml.account_id = account.id
                            AND aml.company_id = account.company_id
                            AND (
                                (account.include_initial_balance AND aml.date <= %(date_to)s)
                                OR (
                                    NOT COALESCE(account.include_initial_balance, FALSE)
