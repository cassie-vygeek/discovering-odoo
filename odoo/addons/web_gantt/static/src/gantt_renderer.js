...
export class GanttRenderer extends Component {
    ...
    toggleRowsReadonly(addReadonly) {
        if (this.extraRows) {
            const extraRow = this.extraRows[this.extraRows.length - 1];
            if (!extraRow.readonly) {
                return;
            }
            const rowIdx = this.rows.findIndex((r) => r.id === extraRow.id);
            this.toggleReadonly(extraRow, addReadonly);
    ...
    toggleReadonly(row, addReadonly) {
        for (const cell of this.getCellsOnRow(row.id)) {
            if (addReadonly) {
