...
QUnit.test("move a pill in another row", async (assert) => {
    ...
    // check readonly attribute has not been added to non-readonly row
    assert.doesNotHaveClass(getCell(1, 1), "o_gantt_readonly");
});
...
QUnit.test("disable drop of pill on groups by readonly field", async (assert) => {
    // Group by: color > cost (readonly) > employee > size
    // Pills can be only be dropped in the same "child groups" (employee & size)
    // or any group above the highest readonly parent (color)

    serverData.models.tasks.fields.cost = {
        string: "cost",
        type: "integer",
    };
    serverData.models.tasks.fields.size = {
        string: "size",
        type: "integer",
    };
    serverData.models.tasks.fields.employee = {
        string: "employee",
        type: "char",
    };
    serverData.models.tasks.fields.cost.readonly = true;
    ...
    assert.hasClass(getPillWrapper("Task 1"), CLASSES.draggable);
    ...
    */
    assert.doesNotHaveClass(getCell(1, 1), "o_gantt_readonly"); // "color" is not readonly
    assert.hasClass(getCell(2, 1), "o_gantt_readonly"); // "cost" is readonly
    assert.hasClass(getCell(3, 1), "o_gantt_readonly"); // "employee"(Jordan) is not readonly but parent "cost" is
    assert.hasClass(getCell(4, 1), "o_gantt_readonly"); // "size"(198) is not readonly but parent "cost" is
    assert.hasClass(getCell(5, 1), "o_gantt_readonly"); // "employee"(Michael) is not readonly but parent "cost" is
    assert.hasClass(getCell(6, 1), "o_gantt_readonly"); // "size"(198) is not readonly but parent "cost" is
    assert.doesNotHaveClass(getCell(7, 1), "o_gantt_readonly"); // "color" is not readonly
    assert.hasClass(getCell(8, 1), "o_gantt_readonly"); // "cost" is readonly
    assert.hasClass(getCell(9, 1), "o_gantt_readonly"); // "employee" is not readonly but parent "cost" is
    assert.hasClass(getCell(10, 1), "o_gantt_readonly"); // "size" is not readonly but parent "cost" is

    const { drop, moveTo } = await dragPill("Task 1");
    await moveTo({ row: 4, column: 2, part: 1 });
    await nextTick();
    ...
    assert.doesNotHaveClass(getCell(1, 1), "o_gantt_readonly");
    assert.hasClass(getCell(2, 1), "o_gantt_readonly");
    assert.doesNotHaveClass(getCell(3, 1), "o_gantt_readonly"); // part of child group
    assert.doesNotHaveClass(getCell(4, 1), "o_gantt_readonly"); // original row of the pill
    assert.doesNotHaveClass(getCell(5, 1), "o_gantt_readonly"); // part of child group
    assert.doesNotHaveClass(getCell(6, 1), "o_gantt_readonly"); // part of child group
    assert.doesNotHaveClass(getCell(7, 1), "o_gantt_readonly");
    assert.hasClass(getCell(8, 1), "o_gantt_readonly");
    assert.hasClass(getCell(9, 1), "o_gantt_readonly"); // NOT part of the child group so should remain readonly
    assert.hasClass(getCell(10, 1), "o_gantt_readonly"); // NOT part of the child group so should remain readonly

    // Drop pill "Task 1" on the same row as "Task 3"
    await moveTo({ row: 6, column: 3, part: 1 });
    await drop({ row: 6, column: 3, part: 1 });
    await nextTick();
    ...
    */
    assert.doesNotHaveClass(getCell(1, 1), "o_gantt_readonly");
    assert.hasClass(getCell(2, 1), "o_gantt_readonly");
    assert.hasClass(getCell(3, 1), "o_gantt_readonly");
    assert.hasClass(getCell(4, 1), "o_gantt_readonly");
    assert.doesNotHaveClass(getCell(5, 1), "o_gantt_readonly");
    assert.hasClass(getCell(6, 1), "o_gantt_readonly");
    assert.hasClass(getCell(7, 1), "o_gantt_readonly");
    assert.hasClass(getCell(8, 1), "o_gantt_readonly");
});
